...
from .forms import Personal_infoForm, FoodCategory, FoodCategoryForm,ChoiceEvaluationForm, ProfileForm
...
# def profileBuilder(request):
#     user_selected = Profile.objects.filter(id = request.session['person_id'])
#     if user_selected:
#         Profile.objects.filter(id=request.session['person_id']).delete()
#     if request.method == 'POST':
#         profileM = Profile()
#         profile = ProfileForm(request.POST)
#         person = request.session['person_id']
#         if profile.is_valid():
#             answer = profile.save(commit=False)
            
#             request.session['person_id'] = answer.id
            
#             print(request.session['person_id'], '-----------------++555555')
            
#             answer.person_id = request.session['person_id']
#             answer = profile.save(commit=True)

#             request.session['person_id'] = answer.id
#             return redirect('Labels_Nudges:recipe_recommendations')
#     else:
#         profile = ProfileForm()
#     return render(request, 'Labels_Nudges/ProfileBuilder.html', context={'form': profile})
